...
class DirectRetrievalEngine(BaseRetrievalEngine):
    ...
    def _get_ollama_embedding(self, text: str, retries: int = 2) -> np.ndarray:
        payload = {"model": self.embed_model, "prompt": text}
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Ollama embedding attempt {attempt + 1} failed ({response.status_code}): {response.text}")
                    if attempt < retries:
                        time.sleep(1)
                        continue
                    response.raise_for_status()
                ...
                if attempt < retries:
                    time.sleep(1)
                    continue
                logger.error(f"Failed to obtain embedding from Ollama for model '{self.embed_model}': {e}")
                raise
